...
func PlanMetadataSchema(ctx context.Context, pool *pgxpool.Pool, root string) (SchemaPlan, error) {
	entities, err := loadMetadataEntities(root)
	if err != nil {
	...
	return BuildMetadataSchemaPlan(entities, live)
}
...
func ApplyMetadataSchemaPlan(ctx context.Context, pool *pgxpool.Pool, plan SchemaPlan) (SchemaSyncResult, error) {
	...
	for _, operation := range plan.Operations {
		if _, err := tx.Exec(ctx, operation.SQL); err != nil {
			return SchemaSyncResult{}, fmt.Errorf("apply metadata schema operation %q: %w", operation.Description, err)
		}
	}
...
func loadMetadataEntities(root string) ([]catalog.LoadedEntity, error) {
	apps, err := registry.New(root).Validate()
	if err != nil {
		return nil, fmt.Errorf("validate apps for metadata schema: %w", err)
	}
	entities, err := catalog.New(apps, fieldtype.DefaultRegistry()).Validate()
	if err != nil {
		return nil, fmt.Errorf("validate entities for metadata schema: %w", err)
	}
	return entities, nil
}
