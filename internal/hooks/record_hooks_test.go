...
func newUserRecordDataMutationQueryer(recordRows ...[]any) *recordDataMutationQueryer {
	queryer := &recordDataMutationQueryer{
		row: newRecordDataMutationRow(int64(10), "core.user", "user", "user", "User", "User identity", "user", false, false, false, false, "", []byte(`{"strategy":"format","format":"{email}"}`), "core", "Core", nil, nil),
		rows: []pgx.Rows{
...
func newLogRecordDataMutationQueryer() *recordDataMutationQueryer {
	return &recordDataMutationQueryer{
		row: newRecordDataMutationRow(int64(2), "core.log", "log", "log", "Log", "Diagnostic log", "file-text", false, false, false, false, "", []byte(`{"strategy":"random","length":16}`), "core", "Core", nil, nil),
		rows: []pgx.Rows{
...
func (q *recordDataMutationQueryer) QueryRow(_ context.Context, sql string, args ...any) pgx.Row {
	...
	if isHookActivityMetadataQuery(sql, args...) {
		return newRecordDataMutationRow(int64(1), "core.activity", "activity", "activity", "Activity", "Timeline entry", "activity", false, false, false, false, "", []byte(`{"strategy":"random","length":16}`), "core", "Core", nil, nil)
	}
	if isHookLogMetadataQuery(sql, args...) {
		return newRecordDataMutationRow(int64(2), "core.log", "log", "log", "Log", "Diagnostic log", "file-text", false, false, false, false, "", []byte(`{"strategy":"random","length":16}`), "core", "Core", nil, nil)
	}
